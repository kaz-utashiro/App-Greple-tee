...
Der Filterbefehl wird als folgendes Argument nach der Moduloption angegeben und endet mit `--`. Zum Beispiel ruft der nächste Befehl den Befehl `tr` mit den Argumenten `a-z A-Z` für das übereinstimmende Wort in den Daten auf.

    greple -Mtee tr a-z A-Z -- '\w+' ...

Der obige Befehl konvertiert alle übereinstimmenden Wörter von Kleinbuchstaben in Großbuchstaben. Eigentlich ist dieses Beispiel nicht sinnvoll, denn **greple** kann dasselbe mit der Option **--cm** effektiver erledigen.
...
Mit der Option **--diskret** wird für jeden übereinstimmenden Teil ein eigener Befehl aufgerufen. Sie können den Unterschied an den folgenden Befehlen erkennen.
...
Sie können sie mit dem Dienst DeepL übersetzen, indem Sie den obigen Befehl mit dem Modul **-Mtee** ausführen und den Befehl **deepl** wie folgt aufrufen:
