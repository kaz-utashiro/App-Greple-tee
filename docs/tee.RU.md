...
Команда фильтрации задается в качестве следующих аргументов после опции модуля, заканчивающейся символом `--`. Например, следующая команда вызывает команду `tr` с аргументами `a-z A-Z` для найденного слова в данных.

    greple -Mtee tr a-z A-Z -- '\w+' ...

Вышеприведенная команда преобразует все найденные слова из нижнего регистра в верхний. На самом деле этот пример бесполезен, потому что **greple** может сделать то же самое более эффективно с помощью опции **--cm**.
...
При использовании опции **--discrete** для каждой сопоставленной части вызывается отдельная команда. Вы можете заметить разницу при выполнении следующих команд.
...
Вы можете перевести их с помощью сервиса DeepL, выполнив приведенную выше команду с помощью модуля **-Mtee**, вызывающего команду **deepl** вот так:
