...
discreteオプションは複数のプロセスを起動するので、プロセスの実行に時間がかかります。そのため、C<--separate ' \r'>オプションとC<ansifold>を併用すると、NLの代わりにCR文字を使って1行にすることができます。
...
B<teip>コマンドで同じことをしようとすると、C<teip -l 2- -- grep>は出力行数が入力行数より少ないのでエラーになります。しかし、得られる結果に問題はありません。
