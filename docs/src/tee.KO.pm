...
필터 명령은 C<-->로 끝나는 모듈 옵션 뒤에 다음 인수로 지정됩니다. 예를 들어, 다음 명령은 데이터에서 일치하는 단어에 대해 C<a-z A-Z> 인수를 사용하여 C<tr> 명령을 호출합니다.

    greple -Mtee tr a-z A-Z -- '\w+' ...

위의 명령은 일치하는 모든 단어를 소문자에서 대문자로 변환합니다. 사실 이 예제는 B<--cm> 옵션을 사용하면 동일한 작업을 더 효과적으로 수행할 수 있으므로 유용하지 않습니다.
...
B<-- 불연속> 옵션을 사용하면 일치하는 각 부분에 대해 개별 명령이 호출됩니다. 다음 명령어를 통해 차이점을 확인할 수 있습니다.
...
위 명령어를 B<-Mtee> 모듈에서 B<deepl> 명령어를 호출하여 DeepL 서비스로 번역하면 다음과 같이 됩니다:
