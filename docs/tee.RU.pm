...
Команда фильтрации задается в качестве следующих аргументов после опции модуля, заканчивающейся символом C<-->. Например, следующая команда вызывает команду C<tr> с аргументами C<a-z A-Z> для найденного слова в данных.

    greple -Mtee tr a-z A-Z -- '\w+' ...

Вышеприведенная команда преобразует все найденные слова из нижнего регистра в верхний. На самом деле этот пример бесполезен, потому что B<greple> может сделать то же самое более эффективно с помощью опции B<--cm>.
...
При использовании опции B<--discrete> для каждой сопоставленной части вызывается отдельная команда. Вы можете заметить разницу при выполнении следующих команд.
...
Вызов новой команды для каждой сопоставленной части.
...
Вы можете перевести их с помощью сервиса DeepL, выполнив приведенную выше команду с помощью модуля B<-Mtee>, вызывающего команду B<deepl> вот так:
