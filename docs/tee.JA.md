...
discreteオプションは複数のプロセスを起動するので、プロセスの実行に時間がかかります。そのため、`--separate ' \r'`オプションと`ansifold`を併用すると、NLの代わりにCR文字を使って1行にすることができます。
...
**teip**コマンドで同じことをしようとすると、`teip -l 2- -- grep`は出力行数が入力行数より少ないのでエラーになります。しかし、得られる結果に問題はありません。
